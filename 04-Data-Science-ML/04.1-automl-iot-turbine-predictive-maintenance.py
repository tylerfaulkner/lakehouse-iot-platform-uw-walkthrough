...
# MAGIC <img width="1px" src="https://ppxrzfxige.execute-api.us-west-2.amazonaws.com/v1/analytics?category=lakehouse&org_id=1655539421928636&notebook=%2F04-Data-Science-ML%2F04.1-automl-iot-turbine-predictive-maintenance&demo_name=lakehouse-iot-platform&event=VIEW&path=%2F_dbdemos%2Flakehouse%2Flakehouse-iot-platform%2F04-Data-Science-ML%2F04.1-automl-iot-turbine-predictive-maintenance&version=1">

# COMMAND ----------

# MAGIC %pip install --quiet databricks-sdk==0.59.0 mlflow==3.1.1 databricks-feature-engineering==0.12.1 plotly seaborn scikit-learn
...
# MAGIC %md
# MAGIC As we can see in these graph, we can clearly see some anomaly on the readings we get from sensor F. Let's continue our exploration and use the std we computed in our main feature table
...
xp_name = f"manual_iot_{datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}"
full_xp_name = f"{xp_path}/{xp_name}"

# Load + sample feature table
training_dataset = (
    fe.read_table(name=f'{catalog}.{db}.turbine_hourly_features')
    .drop('turbine_id')
    .sample(0.1)
    .toPandas()
)
...
training_dataset = training_dataset.sort_values(timestamp_col)  # critical for temporal split
...
# Drop raw timestamp — it's done its job
...
print(f"🔤 String cols being encoded : {string_cols}")
print(f"🔢 Numeric cols passing through: {numeric_cols}")
...
print(f"🕐 Train period: rows 0–{split_idx} | Test period: rows {split_idx}–{len(X)}")

# COMMAND ----------

# OrdinalEncoder is lighter than OneHotEncoder and plays well with tree models
preprocessor = ColumnTransformer(
    transformers=[
        ("ordinal", OrdinalEncoder(handle_unknown="use_encoded_value", unknown_value=-1), string_cols),
    ],
    remainder="passthrough"  # numeric cols ride through untouched
)
...
with mlflow.start_run(run_name="gbt_classifier_timeseries") as run:
    ...
    mlflow.sklearn.log_model(
        pipeline,           # log the whole pipeline, not just the model
        artifact_path="model",
        signature=signature,
...
try:
    DBDemos.set_experiment_permission(full_xp_name)
except:
    pass