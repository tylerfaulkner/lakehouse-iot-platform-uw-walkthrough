...
def get_repo_root():
  #Walk up from the current notebook until we find the demo root folder (works whatever the Git folder is named)
  path = dbutils.notebook.entry_point.getDbutils().notebook().getContext().notebookPath().get()
  folder = os.path.dirname(path)
  while folder not in ("", "/"):
    if os.path.isdir(f"/Workspace{folder}/_resources") and os.path.isdir(f"/Workspace{folder}/01-Data-ingestion"):
      return folder
    folder = os.path.dirname(folder)
  raise Exception(f"Couldn't find the demo root folder (containing config and _resources/) from {path}")
